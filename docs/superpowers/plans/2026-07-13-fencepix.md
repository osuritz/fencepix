...
- Runtime dependencies: exactly `react`, `react-dom`, `zustand`. Dev: `vite`, `typescript`, `@vitejs/plugin-react`, `vitest`, `jsdom`, `@testing-library/react`, `@testing-library/user-event`, `@testing-library/jest-dom`, `fake-indexeddb`. Do not add others.
